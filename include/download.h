 ...
 *   - HTTPS only. Plain HTTP scheme is rejected.
...
/**
 * @brief Extract the host portion of an HTTPS URL into @p out (size @p out_sz).
 *
 * Strips scheme, userinfo, port, and trailing path. Returns 1 on success,
 * 0 if @p url is not `https://...` or hostname is empty.
 */
