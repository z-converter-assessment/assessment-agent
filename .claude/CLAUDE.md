...
- `boot_time` — system boot wall-clock, captured once at process start from the `btime` line of `/proc/stat` and cached for the process lifetime. The kernel folds NTP corrections into `btime`, so re-reading it every cycle would jitter by 1–2s per NTP step and produce false reset events — caching at process start is mandatory. (Earlier agents derived this from `/proc/uptime` + `CLOCK_REALTIME`; the kernel-sourced `btime` is shorter and stays slightly more robust through cold-boot NTP convergence on agent restart.) This field is the **sole authoritative source** for counter-reset detection on the engine. The engine compares `prev.boot_time != curr.boot_time` (change detection only — never absolute-time comparisons like `now() - boot_time < 5min`, which break under clock skew). When changed, the engine MUST skip the differential for that sample (warm-up).
